...
<html>
    <head>
        <meta charset="UTF-8">
        <title> J.CRED | Experian Consumer Profile</title>
    ...
    <body>
        <?php 
        // session_destroy();
        if(isset($_POST['ExperianConProSubmit'])){
            
            $EnquiryReason = $_POST['EnquiryReason'];
            $IDNumber = $_POST['IDnum'];
            $FirstName = $_POST['firstName'];
            $Surname = $_POST['surName'];
            $PassportNumber = $_POST['passportNum'];
            ...
            $_SESSION['passportNumber'] = $PassportNumber;

            $d8 = date('Y-m-d');
                $time = date('h:i:s');
                $cu_email = $_SESSION["email"];
                $action = "Experian Consumer Profile: EnquiryReason=$EnquiryReason;IDNumber=$IDNumber;FirstName=$FirstName;Surname=$Surname;PassportNumber=$PassportNumber ";


$logs = "INSERT INTO logs (Date,Time,Page,Action,user_ID,User) VALUES"."('$d8','$time','Experian Consumer Profile','$action','$cu_id','$cu_email')";
...
';
                ...
                <div class="col-md-10 col-sm-10 con">
                    ...
                    <form action="" method="POST">
                        <h3 class="Trans">Experian Consumer Profile</h3>

                        <ul>
                            <li>
                                <input type="text" id="ref"  name="ref" class="field-style field-split align-right" placeholder="Reference" />
                                <select class="sea-input field-style field-split align-left" id="DeedsOffice" style="height: 36px;" name="EnquiryReason">
                                    ...
                                    <option value="SAPS or State Agency">SAPS or State Agency</option>
                                    <option value="Employment Screening">Employment Screening</option>
                                    <option value="Account Limit Determination">Account Limit Determination</option>
                                    <option value="Application for Insurance">Application for Insurance</option>
                                    <option value="Employment Verification">Employment Verification</option>
                                    <option value="Affordability Assessment">Affordability Assessment</option>
                                    <option value="Debt Review Applications">Debt Review Applications</option>
                            ...
                            <li>
                                <br><br>
                                <input class=" align-right" type="submit" name="ExperianConProSubmit" value="Search" />
                            </li>
                        </ul>
                    ...
                    <div class="seaTable">
                        <table id="personal" class="table table-striped  table-hover">
                            <thead class="head">
                                <tr>
                                    <th scope="col"></th>
...
$(document).ready(function(){
            ...
            console.log('Done1')
                ...
                var compu = '';
                    ...
                    compu += '<td> <a href="experianView.php?x=' +result.ResponseObject.PersonInformation.IDNumber+'&y='  +result.ResponseObject.PersonInformation.Fullname+ '">View<a/></td>';
                    compu += '</tr>';
                    
                


                $('#personal').append(compu);
