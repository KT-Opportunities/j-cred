    ...
    <body>
        <?php 
            ...
            if(isset($_POST['companySearch'])){
                ...
                $CipcSearchBy = $_POST['CipcSearchBy'];
                $_SESSION['CompanyName'] = $CompanyName;
                $_SESSION['RegistrationNumber'] = $regNum;
                $_SESSION['CipcSearchBy'] = $CipcSearchBy;
                $d8 = date('Y-m-d');
                $time = date('h:i:s');
                $cu_email = $_SESSION["email"];
                $action = "Company CIPC Records: Ref=$ref;CompanyName=$CompanyName;regNum=$regNum;CipcSearchBy=$CipcSearchBy";

                $logs = "INSERT INTO logs (Date,Time,Page,Action,user_ID,User) VALUES"."('$d8','$time','Company CIPC Records','$action','$cu_id','$cu_email')";
...
<li>
                ...
                <div class="col-md-10 col-sm-10 con">
                    ...
                    <form action="" method="POST">
                        <h3 class="Trans">Company CIPC Records</h3>

                        <ul>
                            <li>
                                <input type="text" name="ref" class="field-style field-split align-left" placeholder="Reference" />
                                <input type="text" name="CompanyName" class="field-style field-split align-right" placeholder="Company Name" />

                            </li>
                            <li>
                            <select class="sea-input field-style field-split align-left" id="DeedsOffice" style="height: 36px;" name="CipcSearchBy">
                                    <option value="0" selected>Cipc Search By</option>
                                    <option value="1">Exact Match</option>
                                    <option value="2">Contains</option>
                                    <option value="3">Starts With</option>
                                    <option value="4">By Registration Number</option>
                            </select>

                            <input type="text" name="RegistrationNumber" class="field-style field-split align-right" placeholder="Registration Number" />
                            </li> 
                            <li>
                                <input type="submit" name="companySearch" value="Search" />
                            </li>
                        </ul>
    ...
    // $(document).on('click', '#btnS', function() {
            ...
            success:function(result){
                ...
                for (let i = 0; i < result.ResponseObject.length; i++) {
                    console.log('V1: ' ,JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyName));
                    console.log('V2: ' ,JSON.parse(result.ResponseObject[i].CompanyInformation.CompanyID));
                    
                    
                    company += '<tr>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyID).substr(1).slice(0, -1)+ '</td>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyRegistrationNumber).substr(1).slice(0, -1)+ '</td>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyName).substr(1).slice(0, -1)+ '</td>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyTranslatedName).substr(1).slice(0, -1)+ '</td>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.RegistrationDate).substr(1).slice(0, -1)+ '</td>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyStatusCode).substr(1).slice(0, -1)+ '</td>';
                    company += '<td>' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyStatus).substr(1).slice(0, -1)+ '</td>';
                    company += '<td> <a href="companyView.php?x=' +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyID).substr(1).slice(0, -1)+'&y='  +JSON.stringify(result.ResponseObject[i].CompanyInformation.CompanyName).substr(1).slice(0, -1)+ '">View<a/></td>';
                    company += '</tr>';
                    
                    
                }

                $('#table').append(company);
